...
class SchemaYulight0Fluid(SchemaYujoyFluid):
    def __init__(self, dir_in: str, fname_full: str, dir_out: str, sname: str, version: str):
        ...
        n_altered = 0
        set_roots = set()
        with open(os.path.join(self.dir_in, "roots_v3.8.0.txt"), 'r', encoding='utf-8') as fr:
            for line in fr:
                set_roots.add(line.strip())
        for char, codes in self.dict_char_codes.items():
            if char in set_roots:
                # 字根字一律只取两码
                self.dict_char_codes[char] = set(code[:2] for code in codes)
                n_altered += 1
        print("已识别并处理字根编码个数:", n_altered)
        # 2.去除回头码
        n_altered = 0
        dict_char_len = {}  # 每个汉字的字根数
        file_chaifen = os.path.join(self.dir_in, "yulight_chaifen_v3.8.0.dict.yaml")
...
if __name__ == '__main__':
    ...
    # myschema = SchemaYulight0Fluid(
    #     "material_yulight0",
    #     "yulight.full.dict_v3.8.0.yaml",
    #     "schema_yulight0_fluid/dicts_yulight_fluid",
    #     "光华",
    #     "2.4"
    # )
    # myschema.build()
    myschema = SchemaYulight0Fluid(
        "material_yulight0",
        "yulight.full.dict_v3.8.0.yaml",
        "schema_yulight0_fluid/dicts_yulight_fluid_a",
        "光华",
        "2.4"
    )
    myschema.build("A")  # a版
